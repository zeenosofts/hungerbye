                ...
                <div class="box box-primary">
                    ...
                    <div class="box-body">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" v-model="item.item_name" placeholder="e.g Special Combo">
                            <span class="help-block" v-show="error.item_name">Item Name can't be empty!</span>
                        </div>
                        <div class="form-group">
                            <label class="control-label"> Item Description</label>
                            <input type="text" class="form-control" v-model="item.item_description" placeholder="e.g Buy one get second combo on half price">
                    ...
                    <div class="box-footer clearfix no-border">
                        <button type="button" class="btn btn-primary btn-flat pull-right" @click="saveItem()"><i class="fa fa-plus"></i> Add Offer</button>
                    </div>
...
<script>
    ...
    export default {
        ...
        methods: {
            getAllItems(){
                let self=this;
                Promise.resolve(main.sendPOSTRequest('getAllOffers',{id:"nun"})).then(function (response) {
                    self.items=response.data
                });
            },
            editItem(id){
                this.$router.push({'name':'edit-offer',params:{'id':id}});
            },
            deleteItem(id){
                let self=this;
                Vue.dialog
                    ...
                    .then(function(dialog) {
                        Promise.resolve(main.sendPOSTRequest('deleteOffer',{id:id})).then(function (response) {
                            if(response.data == "success"){
                                Vue.$toast.success("Offer deleted successfully");
            ...
            saveItem(){
                ...
                else if(self.item.item_price.trim().length == 0)
                {
                    ...
                    Promise.resolve(main.sendPOSTRequest('saveOffer',self.item)).then(function (response) {
                        if(response.data == "duplicate"){
                            ...
                            self.item_price='';
                            Vue.$toast.success("Offer saved successfully");
                        }else if(response.data == "error"){
                            Vue.$toast.error("Unknown Error Occurred");
                        }
                    });
